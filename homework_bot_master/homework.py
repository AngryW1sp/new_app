...
def check_tokens() -> Literal[True]:
    ...
    flag = True
    tokens = {'Токен практикума': PRACTICUM_TOKEN,
...
def check_response(response):
    ...
    if not isinstance(response, dict):
        raise TypeError(
            f'Не верный тип данных в ответе. На вход поступили данные типа {type(response)}.')
    ...
    if check_tokens() is True:
        ...
        while True:
            try:
                ...
                if error not in errors_logs:
                    errors_logs.append(error)
                    send_message(bot, message)
            finally:
                time.sleep(RETRY_PERIOD)
    else:
        sys.exit('Бот не может продолжать работу! Отсутствуют переменные окружения!')
